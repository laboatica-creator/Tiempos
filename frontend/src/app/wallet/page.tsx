...
export default function WalletPage() {
    ...
    const handleFileShare = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (datos) {
            ...
            if (datos.bancoDetectado) setSelectedSinpeBank(datos.bancoDetectado);
            ...
            if (esTercero) setShowOcrAlert(true);
        }
    ...
    const handleSinpeDeposit = async (e: React.FormEvent) => {
        ...
        try {
            const res = await api.post('/wallet/recharge', {
                ...
                third_party_alert: showOcrAlert,
            ...
            if (res.error) {
                ...
                if (showOcrAlert) {
                    alert('⚠️ ATENCIÓN: Comprobante de tercero. Será revisado manualmente.');
                } else {
    ...
    return (
        <main className="min-h-screen bg-gradient-to-br from-gray-900 to-gray-800 pb-24">
            ...
            <div className="flex bg-gray-800/50 mx-4 mt-6 rounded-2xl p-1">
                <button onClick={() => setActiveTab('recharge')} className={`flex-1 py-3 rounded-xl font-black text-sm transition-all ${activeTab === 'recharge' ? 'bg-emerald-500 text-white shadow-lg' : 'text-gray-400'}`}>📱 Recargar</button>
                <button onClick={() => setActiveTab('withdraw')} className={`flex-1 py-3 rounded-xl font-black text-sm transition-all ${activeTab === 'withdraw' ? 'bg-emerald-500 text-white shadow-lg' : 'text-gray-400'}`}>💸 Retirar</button>
                <button onClick={() => setActiveTab('history')} className={`flex-1 py-3 rounded-xl font-black text-sm transition-all ${activeTab === 'history' ? 'bg-emerald-500 text-white shadow-lg' : 'text-gray-400'}`}>📋 Historial</button>
            </div>
            ...
            {activeTab === 'recharge' && (
                <div className="mx-4 mt-6 bg-white/5 rounded-2xl p-5 border border-white/10">
                    ...
                    <div className="mb-4">
                        ...
                        <label className="w-full py-3 bg-blue-600 rounded-xl text-white font-black text-center text-sm cursor-pointer transition-all hover:bg-blue-700 block">
                            📎 Seleccionar comprobante
                            <input ref={fileInputRef} type="file" accept="image/*,application/pdf" onChange={handleFileShare} className="hidden" />
                        </label>
                        {processing && (
                            <div className="mt-2">
                        ...
                        {ocrError && <p className="text-red-400 text-[10px] mt-1">{ocrError}</p>}
                    </div>
                    ...
                    {showOcrAlert && ocrData && (
                        <div className="mb-4 bg-red-500/20 border border-red-500/50 rounded-xl p-3">
                            ...
                            <p className="text-gray-300 text-[10px] mt-1">El comprobante pertenece a: {ocrData.nombreEmisor || 'Desconocido'} ({ocrData.telefonoEmisor || 'Sin teléfono'})</p>
                            <p className="text-gray-400 text-[9px] mt-1">Será revisado manualmente por el administrador.</p>
                        </div>
                    )}
                    ...
                    <form onSubmit={handleSinpeDeposit} className="space-y-4">
                        <div>
                            ...
                            <input type="number" value={sinpeAmount} onChange={(e) => setSinpeAmount(e.target.value)} placeholder="Ej: 5000" className="w-full bg-gray-800 border border-white/10 p-4 rounded-xl text-white font-black outline-none" required />
                        </div>
                        <div>
                            <label className="text-gray-400 text-xs font-black uppercase block mb-2">Referencia SINPE</label>
                            <input type="text" value={sinpeReference} onChange={(e) => setSinpeReference(e.target.value)} placeholder="Código de la transferencia" className="w-full bg-gray-800 border border-white/10 p-4 rounded-xl text-white font-black outline-none" required />
                        </div>
                        <button type="submit" className="w-full py-4 bg-gradient-to-r from-emerald-600 to-emerald-500 rounded-xl text-white font-black text-sm uppercase tracking-wider">Enviar Solicitud</button>
                    </form>
                </div>
            )}
            ...
            {activeTab === 'withdraw' && (
                <div className="mx-4 mt-6 bg-white/5 rounded-2xl p-5 border border-white/10">
                    ...
                    <form onSubmit={handleWithdrawal} className="space-y-4">
                        <div><label className="text-gray-400 text-xs font-black uppercase block mb-2">Monto a retirar (₡)</label><input type="number" value={withdrawAmount} onChange={(e) => setWithdrawAmount(e.target.value)} placeholder="Mínimo ₡1,000" className="w-full bg-gray-800 border border-white/10 p-4 rounded-xl text-white font-black outline-none" required /></div>
                        <div><label className="text-gray-400 text-xs font-black uppercase block mb-2">Método</label><select value={withdrawMethod} onChange={(e) => setWithdrawMethod(e.target.value)} className="w-full bg-gray-800 border border-white/10 p-4 rounded-xl text-white font-bold outline-none"><option value="SINPE">SINPE Móvil</option><option value="IBAN">Transferencia IBAN</option></select></div>
                        <div><label className="text-gray-400 text-xs font-black uppercase block mb-2">{withdrawMethod === 'SINPE' ? 'Número de teléfono' : 'Número de cuenta IBAN'}</label><input type="text" value={withdrawDetails} onChange={(e) => setWithdrawDetails(e.target.value)} placeholder={withdrawMethod === 'SINPE' ? 'Ej: 506XXXXXXXX' : 'CR00 0000 0000 0000 0000'} className="w-full bg-gray-800 border border-white/10 p-4 rounded-xl text-white font-black outline-none" required /></div>
                        <button type="submit" className="w-full py-4 bg-gradient-to-r from-orange-600 to-orange-500 rounded-xl text-white font-black text-sm uppercase tracking-wider">Solicitar Retiro</button>
                    </form>
                </div>
            )}
                ...
                <div className="mx-4 mt-6 bg-white/5 rounded-2xl p-5 border border-white/10 mb-6">
                    ...
                    <div className="mb-4 space-y-3">
                        <div className="grid grid-cols-2 gap-3">
                            <div><label className="text-gray-400 text-[9px] font-black uppercase block mb-1">Desde</label><input type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} className="w-full bg-gray-800 border border-white/10 p-2 rounded-lg text-white text-sm" /></div>
                            <div><label className="text-gray-400 text-[9px] font-black uppercase block mb-1">Hasta</label><input type="date" value={endDate} onChange={(e) => setEndDate(e.target.value)} className="w-full bg-gray-800 border border-white/10 p-2 rounded-lg text-white text-sm" /></div>
                        </div>
                        <div><label className="text-gray-400 text-[9px] font-black uppercase block mb-1">Tipo de transacción</label><select value={filterType} onChange={(e) => setFilterType(e.target.value)} className="w-full bg-gray-800 border border-white/10 p-2 rounded-lg text-white text-sm"><option value="ALL">Todos</option><option value="DEPOSIT">Depósitos</option><option value="WITHDRAW">Retiros</option><option value="BET">Apuestas</option><option value="WIN">Premios</option><option value="REFUND">Reembolsos</option><option value="BONUS">Bonos</option></select></div>
                    </div>
                    {loading ? <div className="flex justify-center py-10"><div className="animate-spin rounded-full h-8 w-8 border-t-2 border-b-2 border-emerald-500"></div></div> : filteredTransactions.length === 0 ? <div className="text-center py-10"><p className="text-gray-500">No hay transacciones</p></div> : <div className="space-y-3">{filteredTransactions.map((tx, idx) => (<div key={idx} className="bg-gray-800/50 rounded-xl p-4"><div className="flex justify-between items-start"><div><p className={`text-xs font-black uppercase ${tx.type === 'DEPÓSITO' || tx.type === 'DEPÓSITO SINPE' ? 'text-emerald-400' : tx.type === 'RETIRO' ? 'text-orange-400' : tx.type === 'BONUS' ? 'text-yellow-400' : tx.type === 'WIN' ? 'text-green-400' : 'text-blue-400'}`}>{tx.type === 'DEPÓSITO SINPE' ? 'DEPÓSITO' : tx.type}</p><p className="text-gray-400 text-[10px] mt-1">{formatTransactionDate(tx.created_at)}</p><p className="text-gray-500 text-[9px] mt-1 truncate max-w-[150px]">{tx.details || 'Transacción'}</p></div><div className="text-right"><p className={`font-black text-base ${tx.type === 'DEPÓSITO' || tx.type === 'DEPÓSITO SINPE' || tx.type === 'WIN' ? 'text-emerald-400' : tx.type === 'BONUS' ? 'text-yellow-400' : 'text-red-400'}`}>{tx.type === 'DEPÓSITO' || tx.type === 'DEPÓSITO SINPE' || tx.type === 'WIN' || tx.type === 'BONUS' ? '+' : '-'}₡{Number(tx.amount).toLocaleString()}</p><p className={`text-[8px] font-black uppercase mt-1 ${tx.status === 'COMPLETED' || tx.status === 'APPROVED' ? 'text-green-500' : tx.status === 'PENDING' ? 'text-yellow-500' : 'text-red-500'}`}>{tx.status === 'PENDING' ? 'Pendiente' : tx.status === 'COMPLETED' ? 'Completado' : tx.status === 'APPROVED' ? 'Aprobado' : tx.status}</p></div></div></div>))}</div>}
                </div>
            )}
        </main>
    );
